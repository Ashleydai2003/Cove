...
resource "aws_security_group" "rds_sg" {
  ...
  # Inbound rules: Allow access from Lambda security group, EC2 migration instance, Socket.io server, and Matcher Lambda on port 5432
  ingress {
    from_port       = 5432
    to_port         = 5432
    protocol        = "tcp"
    security_groups = [
      ...
      aws_security_group.lambda_matcher_sg.id  # Allow access from Batch Matcher Lambda
    ]
  }

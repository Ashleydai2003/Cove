...
/// CalendarFeed: Manages the user's calendar events with caching and lazy loading.
/// - Handles caching, preloading, and pagination for calendar events.
/// - Use a single shared instance (see AppController).
@MainActor
class CalendarFeed: ObservableObject {
    /// List of calendar events for the user

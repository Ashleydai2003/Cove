//
//  VendorCreateEventView.swift
//  Cove
//
//  Vendor-specific create event view - identical to CreateEventView but uses vendor network calls
//

import SwiftUI

// MARK: - Main View
struct VendorCreateEventView: View {
    // MARK: - Properties
    @EnvironmentObject var vendorController: VendorController
    @Environment(\.dismiss) private var dismiss
    @StateObject private var viewModel = VendorEventModel()
...
extension VendorCreateEventView {
    // MARK: - Header
...
// MARK: - Image Picker Section
    ...
    private func tieredPricingRow(title: String, price: Binding<String>, spots: Binding<String>, icon: String, color: Color) -> some View {
        VStack(spacing: 12) {
            tieredPricingHeader(title: title, icon: icon, color: color)
            tieredPricingInputs(price: price, spots: spots)
        }
        .padding(.horizontal, 16)
        .padding(.vertical, 12)
        .background(tieredPricingBackground)
    }
    
    private func tieredPricingHeader(title: String, icon: String, color: Color) -> some View {
        HStack {
    ...
    private func tieredPricingInputs(price: Binding<String>, spots: Binding<String>) -> some View {
        HStack(spacing: 12) {
            priceInputField(price: price)
            spotsInputField(spots: spots)
        }
    }
    
    private func priceInputField(price: Binding<String>) -> some View {
    ...
    private func spotsInputField(spots: Binding<String>) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("Spots")
                .font(.LibreBodoni(size: 12))
                .foregroundColor(Colors.primaryDark.opacity(0.7))
    ...
    private var tieredPricingBackground: some View {
        RoundedRectangle(cornerRadius: 10)
            .fill(Color.white.opacity(0.4))
            .overlay(
                RoundedRectangle(cornerRadius: 10)
    ...
    private var createButtonView: some View {
        Button {
            viewModel.submitEvent { success in
                if success {
                    // Refresh vendor events feed to show the new event
                    vendorController.refreshVendorEvents()
                    onEventCreated?()
                    dismiss()
                }
            }

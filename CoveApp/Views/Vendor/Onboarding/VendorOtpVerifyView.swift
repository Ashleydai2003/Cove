//
//  VendorOtpVerifyView.swift
//  Cove
//
//  Vendor OTP verification - copied from OtpVerifyView

import SwiftUI

/// View for handling OTP (One-Time Password) verification during vendor onboarding
/// Manages a 6-digit verification code input with automatic field advancement
struct VendorOtpVerifyView: View {
    // MARK: - Properties
    ...
    private var formattedPhoneNumber: String {
        let phoneNumber = UserDefaults.standard.string(forKey: "VendorPhoneNumber") ?? ""
                ...
                VStack {
                    ...
                    HStack {
                        Button {
                            ...
                            VendorOtpVerify.clearVerificationState()
                            ...
                            // Navigate back (VendorPhoneNumberView will auto-send again)
                    ...
                    VStack(alignment: .leading) {
                        ...
                        HStack(spacing: 0) {
                            ...
                            Button {
                                VendorOtpVerify.handleAuthFailure(vendorController: vendorController)
                            } label: {
                                Text("edit number")
                                    .foregroundStyle(Colors.primaryDark)
                                    .font(.LeagueSpartan(size: 15))
    ...
    private func verifyOTP() {
        ...
        let code = otpText
        VendorOtpVerify.verifyOTP(code, vendorController: vendorController) { success in
            isVerifying = false
    ...
    private func resendCode(completion: @escaping (CodeSendResult) -> Void) {
        guard let phoneNumber = UserDefaults.standard.string(forKey: "VendorPhoneNumber") else {
        ...
        let vendorPhone = UserPhoneNumber(number: localNumber, country: Country(id: "0235", name: "USA", flag: "🇺🇸", code: "US", dial_code: "+1", pattern: "### ### ####", limit: 17))
        vendorPhone.sendVerificationCode { result in
            completion(result)
        }
    }
}
